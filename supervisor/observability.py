"""Shared observability helpers for the Itinero supervisor gateway."""
...
def _service_name() -> str:
    return (os.getenv("SERVICE_NAME") or "itinero-supervisor").strip() or "itinero-supervisor"
...
def health_payload(
    *,
    ...
    """Build a health dict with common observability flags (additive)."""
    sentry_on = sentry_active() or bool((os.getenv("SENTRY_DSN") or "").strip())
    body: dict[str, Any] = {
        "status": "ok",
        "service": _service_name(),
        "environment": _app_env(),
        "redis": _redis_configured(),
        "smtp": _smtp_configured(),
        "sentry": sentry_on,
    }
    if dependencies is not None:
        body["dependencies"] = dependencies
        body["status"] = "ok" if dependencies.get("ok", True) else "degraded"
    if configured is not None:
        merged = dict(configured)
        merged.setdefault("redis", _redis_configured())
        merged.setdefault("smtp", _smtp_configured())
        merged.setdefault("sentry", sentry_on)
        body["configured"] = merged
    if extra:
        body.update(extra)
    return body
